...
uint8_t LDC_Init(void){
	...
	if(retVal |= LDC_writeWordAddress8((uint8_t) LDC_DEFAULT_I2CADDR, (uint8_t) RESET_DEV, (uint16_t) VAL_RESET_DEV))
	 {
		return retVal;
	 }
	WAIT1_Waitms(1);
	...
	setcurrentCONFIGvalue(VAL_DEFAULT_CONFIG_START_CONVERSION);
	return retVal;
}
...
uint8_t LDC_writeConfig(uint8_t deviceregister, uint16_t value){
	uint8_t retVal = ERR_OK;
	if(deviceregister == CONFIG) {
		setcurrentCONFIGvalue(value);
	}
	...
	retVal |= LDC_writeWordAddress8(LDC_DEFAULT_I2CADDR, CONFIG, currentCONFIGvalue); // start LDC Conversion
